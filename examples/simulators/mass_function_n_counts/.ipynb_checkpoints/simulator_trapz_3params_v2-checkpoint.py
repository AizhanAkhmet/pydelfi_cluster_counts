...
class Model():
    ...
    def n_counts_trapz(self, theta, z_min, z_max, log10mass_min, log10mass_max, n_steps_z = 100, n_steps_masses = 1000):
        ...
        N_counts_dz = np.zeros((n_steps_z, n_steps_masses))
        ...
        N_counts_dz = np.trapz(N_counts_dz, log10masses_steps, axis = 1)
        N_counts_dz = factor*N_counts_dz
    ...
    def n_counts_trapz_all_new(self, theta, z_min, z_max, log10masses, n_steps_z = 100, n_steps_masses = 1000):
        t1 = time.process_time()
        n_mass_bins = len(log10masses)
        log10masses_steps = np.linspace(log10masses[0:(n_mass_bins-1)], log10masses[1:], num = n_steps_masses)
        log10masses_all = log10masses_steps.T.reshape(((n_mass_bins-1)*n_steps_masses))
        ...
        for j in range(len(z_min)):
            ...
            N_counts_dz = np.zeros((n_steps_z, len(log10masses_all)))
            
            for k in range(n_steps_z):
                N_counts_dz[k, :] = hmf_object_temp.get_mass_function(cosmo_ccl, 10**log10masses_all, scale_fact_a[k])
          
            N_counts_dz = (np.tile(factor,(4000,1)).T*N_counts_dz)
     
            N_counts_dz = N_counts_dz.reshape((n_steps_z, (n_mass_bins-1), n_steps_masses))
    
            for k in range(n_mass_bins - 1):
         
                temp = np.trapz(N_counts_dz[:, k, :], log10masses_steps[:, k], axis = 1)
        
                N_counts_true[k][j] = np.trapz(temp, z_steps)
    ...
    def simulation(self, theta, seed):
        ...
        N_counts_true = self.n_counts_trapz_all_new(theta, self.z_min, self.z_max, self.log10masses)
