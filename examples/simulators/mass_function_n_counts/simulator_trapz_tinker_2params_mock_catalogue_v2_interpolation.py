...
class Model():
    ...
    ############# ~~~~~~~~~~~~ FOR MOCK (M, z) PAIRS ~~~~~~~~~~~~ #############
    def get_Mz_array(self, cosmo_ccl, seed, N, z_min, z_max, M_min, M_max, n_grid_z= 20, n_grid_z_fine = 2000, n_grid_M = 10000):
        np.random.seed(seed) 
        
        z_array_coarse = np.linspace(z_min, z_max, num = n_grid_z)
        masses = np.logspace(M_min, M_max, num =  n_grid_M)
        
        z_array = np.linspace(z_min, z_max, num = n_grid_z_fine)
        
        mass_funct = ccl.halos.hmfunc.MassFuncTinker08(cosmo_ccl)
        hmf = self.halo_mass_function2(mass_funct, cosmo_ccl, masses, z_array_coarse)
        hmf_sums = hmf.sum(axis = 1, keepdims= True)
        prob_norm_M_z_coarse = hmf/hmf_sums
    
        interp_spline = RectBivariateSpline(z_array_coarse, masses, prob_norm_M_z_coarse)
        prob_norm_M_z = interp_spline(z_array, masses)
