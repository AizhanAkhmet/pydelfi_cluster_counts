...
class Model():
    ...
    def __init__(self, log10masses = np.linspace(14, 15.5, num = 2), z_min = np.array([0.1]), 
                 z_max = np.array([1.0]), data_path = 'N_counts_random_data.npy'):
    ...
    def all_n_counts_trapz(self, z_min, z_max, log10masses, theta, n_steps_z=500, n_steps_masses=5000):
    ...
    ############# ~~~~~~~~~~~~ FOR MOCK (M, z) PAIRS ~~~~~~~~~~~~ #############
    def get_Mz_array(self, cosmo_ccl, seed, N, z_min, z_max, M_min, M_max, n_grid_z= 91, n_grid_M = 5000):
        np.random.seed(seed) 
        ...
        indices_values = np.random.choice(indices_array, size = N, p = prob_norm_Mz_flat)
