...
class Model():
    ...
    def simulation(self, theta, seed):
        ...
        z_array = np.linspace(self.z_min[0], self.z_max[-1], num = 10000)
        #z_array = np.concatenate(z_array)
        
        results = self.get_Mz_array(cosmo_ccl, seed, np.sum(N_counts_true), z_array, self.log10masses[0], self.log10masses[-1]) 
        #           for i in np.arange(len(N_counts_true))]
        #results = [self.get_Mz_array(cosmo_ccl, seed, N_counts_true[i], z_array, self.log10masses[0], self.log10masses[-1]) 
        #           for i in np.arange(len(N_counts_true))]
        ...
        return results #results_bins
    
    ############# ~~~~~~~~~~~~ FOR MOCK (M, z) PAIRS ~~~~~~~~~~~~ #############
    def get_Mz_array(self, cosmo_ccl, seed, N, z_array, M_min, M_max, n_grid_z= 100, n_grid_M = 10000):
        np.random.seed(seed) 
        ...
        prob_z = np.ones(len(z_array)).reshape(len(z_array), 1)
