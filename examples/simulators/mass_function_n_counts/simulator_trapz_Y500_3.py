...
class Model():
    ...
    def compute_probability_Y_500_array(self, mean_Y_500, sigma_logY_500, Y_500_true): 
        prob_Y_500_true = np.exp(-((np.log(Y_500_true) - np.log(mean_Y_500))**2)/(2*(sigma_logY_500**2)))/(np.sqrt(2*np.pi)*sigma_logY_500)
    ...
    def compute_Y500_integral(self, index, Y_bins_limits, mean_Y_500_flat2, sigma_logY_500_flat2, n_steps_M_500, n_steps_z, n_steps_Y):
        ...
        Y_bins_min = np.log(Y_bins_limits[index])
        Y_bins_max = np.log(Y_bins_limits[index+1])
        ...
        prob_Y_500_array = self.compute_probability_Y_500_array(mean_Y_500_flat2, sigma_logY_500_flat2, np.exp(Y_500_array_flat)).reshape(n_steps_Y, n_steps_Mz)
    ...
    def all_n_counts_trapz(self, z_min, z_max, Y_bins_limits, theta, theta_mass_calibration, 
                           n_steps_z=500, n_steps_M_500 = 100, n_steps_Y = 100):
        
        n_bins_z = len(z_min)
        n_steps_Mz = n_steps_z*n_steps_M_500 
        
        #log10_M_500_steps = np.linspace(6, 16, num = n_steps_M_500)
        log10_M_500_steps = np.linspace(14.0, 15.5, num = n_steps_M_500)
